...
idc=`aws organizations list-delegated-administrators --service-principal sso.amazonaws.com --region $REGION --output json --profile $AWS_PROFILE | jq -r '.DelegatedAdministrators[] | select(.Id=='\"$TEAM_ACCOUNT\"') | .Id'`
cloudtrail=`aws organizations list-delegated-administrators --service-principal cloudtrail.amazonaws.com --region $REGION --output json --profile $AWS_PROFILE | jq -r '.DelegatedAdministrators[] | select(.Id=='\"$TEAM_ACCOUNT\"') | .Id'`
accountManager=`aws organizations list-delegated-administrators --service-principal account.amazonaws.com --region $REGION --output json --profile $AWS_PROFILE | jq -r '.DelegatedAdministrators[] | select(.Id=='\"$TEAM_ACCOUNT\"') | .Id'`
serviceRole=`aws iam get-role --role-name AWSServiceRoleForCloudTrail --region $REGION --profile $AWS_PROFILE ` 
...
aws organizations enable-aws-service-access --service-principal cloudtrail.amazonaws.com --region $REGION --profile $AWS_PROFILE

if [ -z "$serviceRole" ]; then
    aws iam create-service-linked-role --aws-service-name cloudtrail.amazonaws.com --region $REGION --profile $AWS_PROFILE
fi
...
aws organizations enable-aws-service-access --service-principal sso.amazonaws.com  --region $REGION --profile $AWS_PROFILE
