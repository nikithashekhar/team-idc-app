...
if [ -z "$SECRET_NAME" ]; then
  aws codecommit create-repository --region $REGION --repository-name team-idc-app --repository-description "Temporary Elevated Access Management (TEAM) Application" --profile $AWS_PROFILE
  ...
  if [[ ! -z "$TAGS" ]]; then
    if [[ ! -z "$UI_DOMAIN" ]]; then
      aws cloudformation deploy --region $REGION --template-file template.yml \
        ...
        --tags $TAGS \
        --no-fail-on-empty-changeset --capabilities CAPABILITY_NAMED_IAM --profile $AWS_PROFILE
    else
      aws cloudformation deploy --region $REGION --template-file template.yml \
        --stack-name TEAM-IDC-APP \
        --parameter-overrides \
